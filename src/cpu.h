...
public:
    ...
    void clock(); // Reset Interrupt - Forces CPU into known state
    void reset(); // Interrupt Request - Executes an instruction at a specific location
    void irq(); // Non-Maskable Interrupt Request - As above, but cannot be disabled
    void nmi(); // Perform one clock cycle's worth of update
